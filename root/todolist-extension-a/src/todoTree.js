...
class TodoTreeProvider {
    constructor(tasks) {
        ...
        this._onDidChangeTreeData = new class {
            ...
            event() { return (listener) => { this.listeners.push(listener); return () => { this.listeners = this.listeners.filter(l => l !== listener); }; }; }
        }();
    ...
    getTreeItem(element) {
        ...
        treeItem.command = { command: 'todolist-extension-a.openTaskFile', title: 'Open Task Resource', arguments: [element] };
        treeItem.contextValue = 'task';
        return treeItem;
    }

    getChildren(element) { return element ? [] : this.tasks; }

    refresh() {
        this._onDidChangeTreeData.fire();
        if (global.webviewPanel) global.webviewPanel.webview.html = global.getDashboardContent(this.tasks);
    }
}
